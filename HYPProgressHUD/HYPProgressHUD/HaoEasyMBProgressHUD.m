...
#define MBMainThreadAssert() NSAssert([NSThread isMainThread], @"HaoEasyMBProgressHUD needs to be accessed on the main thread.");

CGFloat const MBProgressMaxOffset2 = 1000000.f;
...
@interface HaoEasyMBProgressHUD () {
    // Deprecated
...
@interface MBProgressHUDRoundedButton2 : UIButton
@end


@implementation HaoEasyMBProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    HaoEasyMBProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    HaoEasyMBProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (HaoEasyMBProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            return (HaoEasyMBProgressHUD *)subview;
        }
    }
    return nil;
}
...
- (void)done {
    ...
    id<MBProgressHUDDelegate2> delegate = self.delegate;
...
- (void)setupViews {
    ...
    MBBackgroundView2 *backgroundView = [[MBBackgroundView2 alloc] initWithFrame:self.bounds];
    ...
    MBBackgroundView2 *bezelView = [MBBackgroundView2 new];
    ...
    UIButton *button = [MBProgressHUDRoundedButton2 buttonWithType:UIButtonTypeCustom];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[MBRoundProgressView2 class]];
    ...
    if (mode == MBProgressHUDModeIndeterminate) {
        ...
        [indicator removeFromSuperview];
        indicator = [[MBBarProgressView2 alloc] init];
        [self.bezelView addSubview:indicator];
    }
        ...
        if (!isRoundIndicator) {
            ...
            [indicator removeFromSuperview];
            indicator = [[MBRoundProgressView2 alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == MBProgressHUDModeAnnularDeterminate) {
            [(MBRoundProgressView2 *)indicator setAnnular:YES];
        }
    } 
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[HaoEasyMBProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[HaoEasyMBProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[MBRoundProgressView2 class]]) {
        MBRoundProgressView2 *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MBRoundProgressView2 appearanceWhenContainedIn:[HaoEasyMBProgressHUD class], nil];
#else
        appearance = [MBRoundProgressView2 appearanceWhenContainedInInstancesOfClasses:@[[HaoEasyMBProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((MBRoundProgressView2 *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((MBRoundProgressView2 *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[MBBarProgressView2 class]]) {
        MBBarProgressView2 *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MBBarProgressView2 appearanceWhenContainedIn:[HaoEasyMBProgressHUD class], nil];
#else
        appearance = [MBBarProgressView2 appearanceWhenContainedInInstancesOfClasses:@[[HaoEasyMBProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((MBBarProgressView2 *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((MBBarProgressView2 *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    MBBackgroundView2 *bezelView = self.bezelView;
...
@implementation MBBarProgressView2
...
@implementation MBBackgroundView2
...
@implementation HaoEasyMBProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [HaoEasyMBProgressHUD allHUDsForView:view];
    for (HaoEasyMBProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (BOOL)dimBackground {
    MBBackgroundView2 *backgroundView = self.backgroundView;
